...
class PersonDao {
    ...
    create( vorname = '', nachname = '', adresseid = 1, telefonnummer = '') {
        var sql = 'INSERT INTO Person (Vorname,Nachname,AdresseID,Telefonnummer) VALUES (?,?,?,?,?,?,?)';
        var statement = this._conn.prepare(sql);
        var params = [(vorname, nachname, adresseid, telefonnummer)];
    ...
    update(id, vorname = '', nachname = '', adresseid = 1, telefonnummer = '') {
        var sql = 'UPDATE Person SET Vorname=?,Nachname=?,AdresseID=?,Telefonnummer=? WHERE ID=?';
        var statement = this._conn.prepare(sql);
        var params = [( vorname, nachname, adresseid, telefonnummer), id];
        var result = statement.run(params);
